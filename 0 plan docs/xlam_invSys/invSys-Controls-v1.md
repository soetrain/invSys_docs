...
remain unchanged. The current runtime coverage step is the existing Admin UOM
Add/Remove/Reset commands, specified under D18.
Their initial packaged RED is 58 PASS / 68 expected FAIL: owning effects and all
43 setup/baseline checks pass, while eleven outcomes lack activity evidence and
held forms fail session/target binding. One of three inspected native captures
is rejected for a blank body. The isolated implementation now passes **126/126**,
with all five packages built/compiled and Operations cold start passing. All three
fresh native confirmations are directly reviewed and hashed; Excel closes normally,
source/package pins remain unchanged and no matching Application errors occur.
Static metrics are 249 components / 6,036 procedures / 132,551 lines, dynamic calls
9/45 and duplicate groups 192; all 28 prior limits pass. The missing Cancelled
choice in the expectation editor has its own actual-editor RED: 96 PASS and
exactly two expected failures, selecting and retaining the authored cancelled
step. Native cancellation, unchanged Config and existing checks pass; all six
captures are reviewed. The catalog-filtered candidate fix now passes **98/98**
on the new isolated package set, with all five builds/compiles, Operations cold
start, six reviewed fresh captures and normal Excel closure. Static metrics and
all 28 limits remain unchanged; all three report schemas pass.
The expanded Admin UOM gate is **225/225**, retaining all 126 prior checks.
Catalog compatibility, all twelve published action pairs, exact original
observations and policy/unknown-column preservation pass. Actual commands retain
their results with unavailable storage, older policy or disabled collection;
tracking notices match availability. Nine fresh confirmations are reviewed.
Normal closure, unchanged source/package pins and no matching Application errors
are verified for both 98/98 and 225/225 gates. Broader regression/release gates
remain pending; no operator acceptance is inferred from these tests.
The Settings regression retains all **191/191** prior checks, including restart,
Operations-only preferences and native maximize/restore. Ten captures are reviewed;
nine are usable. The initial General-tab image has unpainted controls, so fresh
...
22 Boxing/Shipping images were reviewed. Individual-file inspection confirms
...
family/field enablement and order, locks required context, previews synthetic
values and exposes its own Save/Reset/Reload. D18's schema refinement describes
the complete profile snapshot and Core-owned Config tables. Surface RED is
64 PASS / 8 FAIL; clean no-write Save RED is 94 PASS / 3 FAIL. The corrected
candidate passes all 42 profile checks: 105 PASS / 2 FAIL overall, retaining every
prior GREEN. Save/reload, append-only versions, unknown columns, cancellation and
malformed-latest defaults pass. Five packages compile and smoke is 86/86; the
version-2 editor capture was inspected. Full Receiving remains 854/854; static
limits and all 90 prior package / 16 protected source pins hold.
Viewer consumption, preferences,
comprehensive activity and human acceptance remain open.

**2026-09-13, Slice 4be.2, previous cancelled-save correction:** A real Excel BeforeSave
...
and Reload reads the current policy. Packaged policy RED is 44 PASS / 3 FAIL;
the expanded candidate is 54 PASS / 2 FAIL, including all 25 policy and 18 D5
checks. Detail profiles, personal preferences, policy-save observations and
comprehensive new-control activity coverage remain pending. The version-2
saved-policy capture was inspected; this is not human or Slice 4be acceptance.
Full Receiving remains 854/854 and packaged smoke 86/86; five packages compile,
static limits hold and preserved package/source pins match.
The governing D18 contract is unchanged. See the linked Settings evidence.

**2026-09-13, Slice 4be.2, previous UI checkpoint:** General/Event Tracking tabs
...
Catalogs1-6 and prior controls remain unchanged. Native115/115 protects
...
isolated Production/restart2/2 pass; the unchanged clean full chain passes30/30
...
source changed in this worksheet checkpoint.
...
The implemented publisher creates a hash-verified immutable release and only
then advances `current-release.json`; the implemented station updater runs only
with Excel closed, copies into a side-by-side local cache, and repoints only
the Operations/Admin leaf registration. It automatically defers on an open
Excel process and restores the prior registration/pointer if registration
fails. Manual rollback requires a local Windows administrator, an explicit
...
and visibly accepted:** **Server Sign In** must not report a remembered NAS root
...
are GREEN. On 2026-09-04, visible operator UAT passed NAS sign-in followed by
the separate invSys sign-in.
